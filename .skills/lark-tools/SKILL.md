...
`lark-cli` is on `$PATH`. **Always run this check first**; if missing, install it for the user
...
- The npm package is **`@larksuite/cli`** (provides the `lark-cli` binary). Do not guess other
  package names.
- If the global install fails with `EACCES` (permission denied on system npm prefix), tell the
  user and offer either `sudo npm install -g @larksuite/cli` or a per-user prefix
  (`npm config set prefix ~/.npm-global` and add `~/.npm-global/bin` to `$PATH`). **Never run
  `sudo` without telling the user first.**
...
Always use `${DAEMON_BASE}` for the **onboard endpoints** below — `lark-cli` cannot drive
app-registration, so the daemon owns that one flow. Everything else (auth status, bot
binding check, calendar / IM / mail / …) goes through `lark-cli` directly, no `curl` to
the daemon needed.
...
the `/mavis` prefix) falls through to the SPA HTML and returns garbage HTML to your `jq`
pipe — do not strip the prefix.
...
```bash
lark-cli auth status 2>/dev/null | jq '{appId, identity, userOpenId, userName, tokenStatus, scope}'
```
...
```bash
# Step 1: Start onboarding (returns the QR for app-registration consent)
...
```bash
# Step 2: Poll status — wait `intervalSec` between calls
...
```bash
# Cancel if the user closes the dialog or you want to abort
curl -s -X POST "${DAEMON_BASE}/mavis/api/lark/onboard/cancel" \
  -H 'Content-Type: application/json' \
  -d "{\"sessionId\":\"${SESSION_ID}\"}"
```

When `status` becomes `done`, the daemon has written:
- `~/.lark-cli/config.json` (the global lark-cli multi-app config)
- `~/Library/Application Support/lark-cli/appsecret_<appId>.enc` (encrypted appsecret)
- `~/Library/Application Support/lark-cli/<appId>_<openId>.enc` (encrypted UAT — unless
  `skipUserAuth: true`)

so both app-level **and** user-level `lark-cli` calls work immediately. **Only when an
operation later requires a high-sensitivity scope outside the recommended set** does an
agent need to run `lark-cli auth login --scope "..."` — see
...
shared with the terminal `lark-cli`:

- `~/.lark-cli/config.json` — multi-app config; the bound bot appears as `apps[0]`.
- `~/Library/Application Support/lark-cli/appsecret_<appId>.enc` — encrypted appSecret.
- `~/Library/Application Support/lark-cli/<appId>_<openId>.enc` — encrypted UAT.
...
```bash
lark-cli auth status 2>/dev/null | jq '{appId, identity, userOpenId, userName, tokenStatus, scope, expiresAt}'
```
