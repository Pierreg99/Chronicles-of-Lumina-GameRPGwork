...
- **`--as user`（推荐）**：以当前登录用户身份操作其有权访问的 Base。在 mavis 中，daemon onboard 已自动跑过 `lark-cli auth login --recommend` 拿到推荐 scope 全集（含 base 常用读写 scope），通常无需再手动登录；若仍提示缺特定 base scope，再 background 跑 `lark-cli auth login --scope "<missing_scope>"` 增量补：

```bash
# 增量补 scope（仅当报错明确缺某个推荐范围之外的 base scope 时）
lark-cli auth login --scope "base:record:update"
```
