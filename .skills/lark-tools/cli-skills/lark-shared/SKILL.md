...
> - **运行时检测到缺 scope**：根据错误提示**直接** background 跑 `lark-cli auth login --scope "<missing_scopes>"` 增量授权，从输出中提取 device URL 发给用户。**不要** curl daemon、不要再用 `--domain`、不要让用户自己跑命令。仅在推荐 scope 集合之外的高敏感 scope 才会触发这条路径。
...
当你作为 AI agent 需要帮用户**增量**完成认证时（即推荐 scope 之外的高敏感 scope），**background 方式执行 `lark-cli auth login --scope "..."`，从输出中提取授权链接发给用户**：

```bash
# 增量授权某个具体 scope（阻塞直到用户授权完成或过期）
lark-cli auth login --scope "search:message"
```

> 在 mavis 中，**首次**授权（`--recommend` 拿推荐 scope 全集）已经在 daemon onboard 完成时自动跑过，UI 直接展示了授权 URL。这里的 `--scope` 仅用于**运行时增量补 scope**——**不要**再发起 `--recommend`，否则会重复弹授权窗口骚扰用户。

授权完成后输出会写入全局 lark-cli store（`~/.lark-cli/` + `~/Library/Application Support/lark-cli/*.enc`），daemon 端 IM bot 和终端 lark-cli 共享同一份凭证，不需要 daemon 介入。
