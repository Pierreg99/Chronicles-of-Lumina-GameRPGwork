...
- **认证** — 列出所需 scope。在 mavis 中 daemon onboard 已自动跑过 `lark-cli auth login --recommend` 拿到推荐 scope 全集；若你的 skill 用到推荐范围之外的高敏感 scope，文档里写明 `lark-cli auth login --scope "<scope>"` 增量补即可，不要再教 `--domain`。
