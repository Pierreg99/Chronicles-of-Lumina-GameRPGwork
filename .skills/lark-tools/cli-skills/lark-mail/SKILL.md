...
- **`--as user`（推荐）**：以当前登录用户的身份访问其邮箱。在 mavis 中，daemon onboard 已经自动跑过 `lark-cli auth login --recommend` 拿到推荐 scope 全集（含 mail），通常无需再手动登录；若仍报 `LARK_USER_AUTH_REQUIRED` 或缺特定 mail scope，再 background 跑 `lark-cli auth login --scope "<missing_scope>"` 增量补。
- **`--as bot`**：以应用身份访问邮箱。需要在飞书开发者后台为应用开通相应权限，否则请求会被拒绝。**注意：bot 身份仅适用于读取类操作，所有写操作（发送、回复、转发、草稿编辑等）仅支持 user 身份。**

1. 所有邮件写操作（发送、回复、转发、草稿编辑） → 必须使用 `--as user`，未登录或缺 scope 时按 `lark-shared` 中的"权限不足处理"流程操作（首次走 `--recommend`，增量走 `--scope`）
